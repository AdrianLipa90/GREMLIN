...
def _authority() -> dict[str, bool]: return {"production_runtime_write": False, "execution_admitted": False, "canon_allowed": False}
...
def _commit(domain: bytes, value: Any) -> str: return hashlib.blake2b(domain + b"\0" + _canonical(value), digest_size=32).hexdigest()
def _norm(text: str) -> str: return str(text).strip().replace("×", "*").replace("·", "*").replace("⋅", "*").replace("−", "-").replace("–", "-")
...
def _cx(r): return (float(r["bbox"][0])+float(r["bbox"][2]))/2.0
def _cy(r): return (float(r["bbox"][1])+float(r["bbox"][3]))/2.0
def _height(r): return max(float(r["bbox"][3])-float(r["bbox"][1]),1e-9)
def _x_overlap(a,b):
    ax0,_,ax1,_=[float(v) for v in a["bbox"]]; bx0,_,bx1,_=[float(v) for v in b["bbox"]]
    return max(0.0,min(ax1,bx1)-max(ax0,bx0))/max(min(ax1-ax0,bx1-bx0),1e-9)
def _operand_like(t):
    v=str(t).strip(); return bool(v) and v not in _OPERATORS and v not in {"(","[","{"}
...
def _join_tokens(tokens:list[str])->str:
    ...
    if not clean:return ""
    out=clean[0]; previous=clean[0]
    for token in clean[1:]:
        if token in {')',']','}'} or _closing_with_script(token): out+=token
        elif previous in {'(','[','{'}: out+=token
        elif _operand_like(previous) and _operand_like(token): out+="*"+token
        else: out+=" "+token
        previous=token
...
def _nearest_left_base(script:Mapping[str,Any], main:list[dict[str,Any]], *, reference_size:float):
    sx0=float(script["bbox"][0]); limit=max(6.0,reference_size*0.85)
    candidates=[r for r in main if float(r["bbox"][2])<=sx0+1.0 and sx0-float(r["bbox"][2])<=limit]
    if not candidates:return None
    # Horizontal proximity alone can bind a superscript to a neighbouring lower line.
    # Prefer the base on the geometrically nearest vertical level, then the nearest left edge.
...
def _group_scripts(rows:list[dict[str,Any]], reference_size:float):
    ...
    small=sorted([dict(r) for r in rows if float(r["size"])<=threshold],key=lambda r:(float(r["bbox"][0]),_cy(r)))
    main=[dict(r) for r in rows if float(r["size"])>threshold]; constructs=[]
    ...
    for base in main:
        glyphs=sorted(assigned.get(id(base),[]),key=lambda r:(float(r["bbox"][0]),_cy(r)))
        ...
        for side,group in sides.items():
            ...
            if max(centers)-min(centers)>max(2.0,reference_size*0.35):unattached.extend(group);continue
            ordered=sorted(group,key=lambda r:float(r["bbox"][0])); script_text="".join(_norm(r["text"]) for r in ordered)
...
def _y_clusters(rows,tolerance):
    ...
    for row in ordered[1:]:
        ...
        (clusters[-1] if abs(_cy(row)-center)<=tolerance else clusters.append([row]))
    return clusters
...
def _has_explicit_connector(rows): return any(_norm(r["text"]) in _OPERATORS for r in rows)
...
def _render_segment(rows:list[dict[str,Any]],reference_size:float):
    ...
    tolerance=max(2.0,statistics.median([_height(r) for r in scripted])*0.42); clusters=_y_clusters(scripted,tolerance)
    ...
    if max((_x_overlap(a,b) for a in upper for b in lower),default=0.0)<0.20:return None,constructs,"VERTICAL_LEVELS_NOT_FRACTION_ALIGNED"
    # Two separated numerator/denominator columns with no explicit connector are not one fraction.
    # Fail closed rather than inventing multiplication between independent vertical stacks.
    if len(upper)>1 and len(lower)>1 and not _has_explicit_connector(upper) and not _has_explicit_connector(lower):
        upper_x=sorted(float(r["bbox"][0]) for r in upper); lower_x=sorted(float(r["bbox"][0]) for r in lower)
        gap_limit=max(8.0,reference_size*1.5)
        if any(b-a>gap_limit for a,b in zip(upper_x,upper_x[1:])) and any(b-a>gap_limit for a,b in zip(lower_x,lower_x[1:])):
            return None,constructs,"MULTIPLE_UNCONNECTED_VERTICAL_STACKS"
    numerator=_render_horizontal(upper); denominator=_render_horizontal(lower)
    if not numerator or not denominator:return None,constructs,"EMPTY_FRACTION_COMPONENT"
    constructs.append("STACKED_FRACTION"); return f"({numerator})/({denominator})",constructs,None
...
def _status(*,status,linear_text,relation_count,constructs,flags,rows,page_number,equation_label):
    ...
    core["solver_commitment"]=_commit(b"GREMLIN-COMPOSITE-2D-MATH/v0.1",core);return core
...
def solve_composite_2d_equation(spans:Iterable[Mapping[str,Any]],*,page_number:int,equation_label:str)->dict[str,Any]:
    ...
    content=[r for r in rows if r["text"]!=label]; relation_rows=sorted([r for r in content if _norm(r["text"]) in _RELATIONS],key=lambda r:float(r["bbox"][0]))
    if not relation_rows:return _status(status="AMBIGUOUS_COMPOSITE_2D_UNRESOLVED",linear_text=None,relation_count=0,constructs=[],flags=["NO_EXPLICIT_TOP_LEVEL_RELATION"],rows=rows,page_number=page,equation_label=label)
    reference_size=statistics.median(float(r["size"]) for r in relation_rows);segments=[];left_bound=-math.inf
    for relation in relation_rows:
        right_bound=float(relation["bbox"][0]);segments.append([r for r in content if r not in relation_rows and _cx(r)>left_bound and _cx(r)<right_bound]);left_bound=float(relation["bbox"][2])
    segments.append([r for r in content if r not in relation_rows and _cx(r)>left_bound])
    rendered=[];constructs=[];flags=[]
    for index,segment in enumerate(segments):
        text,found,error=_render_segment(segment,reference_size);constructs.extend(found)
        if error or not text:
            flags.append(f"SEGMENT_{index}_{error or 'UNRENDERABLE'}");return _status(status="AMBIGUOUS_COMPOSITE_2D_UNRESOLVED",linear_text=None,relation_count=len(relation_rows),constructs=constructs,flags=flags,rows=rows,page_number=page,equation_label=label)
        rendered.append(text)
    output=rendered[0]
    for relation,segment_text in zip(relation_rows,rendered[1:]):output+=f" {_norm(relation['text'])} {segment_text}"
    return _status(status="SOLVED_COMPOSITE_2D",linear_text=output,relation_count=len(relation_rows),constructs=constructs,flags=["GEOMETRIC_COMPOSITE_RECONSTRUCTION"],rows=rows,page_number=page,equation_label=label)
