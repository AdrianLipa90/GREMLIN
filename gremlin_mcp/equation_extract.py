...
_SCI_RE = re.compile(r"(?P<mant>[+-]?(?:\d+(?:\.\d*)?|\.\d+))\s*\*\s*10\s*\*\*\s*(?P<exp>[+-]?\d+)")
_REPORTED_QUANTITY_RE = re.compile(r"^(?P<number>[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?)(?:\s*\*\s*(?P<unit>[A-Za-z][A-Za-z0-9_]*(?:\*\*[+-]?\d+)?(?:[*/][A-Za-z][A-Za-z0-9_]*(?:\*\*[+-]?\d+)?)*))?$")
...
def propose_equation_witnesses(text: str, *, source_id: str) -> dict[str, Any]:
    ...
    for row in formula_spans:
        ...
        bound = {s: constants[s] for s in row["symbols"] if s in constants}
    ...
    for row in relation_chain_spans:
        missing = sorted(s for s in row["symbols"] if s not in constants)
        basis = {"label": row["label"], "expression": row["expression"], "reported": row["reported_value"], "reported_unit": row["reported_unit"], "extraction_basis": row["extraction_basis"]}
        common = {"id": _proposal_id(source, "numeric_relation_chain", basis), "kind": "numeric", "expression": row["expression"], "reported_value": row["reported_value"], "reported_unit": row["reported_unit"], "source_locator": f"{source}:{row['label']}", "source_excerpt": row["source_excerpt"], "extraction_basis": row["extraction_basis"], "epistemic_status": "CANDIDATE_WITNESS"}
        bound = {s: constants[s] for s in row["symbols"] if s in constants}
        (unresolved if missing else proposals).append({**common, **({"missing_symbols": missing} if missing else {}), "symbols": bound})
