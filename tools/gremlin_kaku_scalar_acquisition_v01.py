...
SEMANTIC_CONTRACTS = {
    ...
    "affect": {
        ...
        "support_term_ids": ("CLX2-AFFECT-001",),
    },
    "intention_alignment": {
...
def _semantic(role: str) -> Mapping[str, Any]:
    try:
...
def build_producer_contract(
    *,
    ...
    semantic = _semantic(semantic_role)
    ...
    return {**core, "producer_contract_commitment": _commit(PRODUCER_DOMAIN, core)}
...
def validate_producer_contract(contract: Mapping[str, Any]) -> bool:
    ...
    role = str(contract.get("semantic_role", ""))
    semantic = _semantic(role)
    ...
    if contract.get("live_required") is True and contract.get("source_classification") != "LIVE_NOEMA_WITNESS":
    ...
    if supplied != _commit(PRODUCER_DOMAIN, core):
...
def build_observation_receipt(
    *,
    ...
    return {**core, "receipt_id": _commit(RECEIPT_DOMAIN, core)}
...
def validate_observation_receipt(receipt: Mapping[str, Any]) -> bool:
    ...
    if supplied != _commit(RECEIPT_DOMAIN, core):
...
def build_acquisition_bundle(receipts: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    expected = set(SEMANTIC_CONTRACTS)
    if set(by_role) != expected:
        raise GremlinScalarAcquisitionError(
            f"exact scalar role set required; missing={sorted(expected - set(by_role))}, extra={sorted(set(by_role) - expected)}"
        )
    ...
    return {**core, "acquisition_bundle_commitment": _commit(BUNDLE_DOMAIN, core)}
...
def validate_acquisition_bundle(bundle: Mapping[str, Any]) -> bool:
    ...
    if roles != sorted(SEMANTIC_CONTRACTS) or len(set(roles)) != len(roles):
        raise GremlinScalarAcquisitionError("acquisition observations require canonical unique role ordering")
    for observation in observations:
        semantic = _semantic(str(observation["semantic_role"]))
        ...
        _digest(observation.get("receipt_id"), "receipt_id")
    ...
    supplied = _digest(bundle.get("acquisition_bundle_commitment"), "acquisition_bundle_commitment")
    core = dict(bundle)
...
def build_kaku_scalar_packet_from_acquisition(
    *,
    ...
    lineage = [f"receipt:{item['receipt_id']}" for item in acquisition_bundle["observations"]]
    lineage.append(f"acquisition:{acquisition_bundle['acquisition_bundle_commitment']}")
    ...
    packet = build_kaku_scalar_packet(
        ...
        valuation=observation("valuation"),
        affect=observation("affect"),
        intention_alignment=observation("intention_alignment"),
        epistemic_support=observation("epistemic_support"),
