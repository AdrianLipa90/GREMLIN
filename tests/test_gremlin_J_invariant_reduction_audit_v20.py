...
def test_mass_spectrum_only_gives_conditional_reduction_until_source_operator_is_bound():
    receipt = build_J_invariant_reduction_audit_v20(**BASE)
    mass = receipt["candidates"]["nondegenerate_mass_spectrum"]
    assert mass["paired_source_operator_M_I_status"] == "UNRESOLVED"
    assert mass["conditional_stabilizer"] == "U(1)^3"
    assert mass["conditional_stabilizer_dimension"] == 3
    assert mass["conditional_projective_dimension"] == 2
    assert mass["verdict"] == "CONDITIONAL_REDUCTION_REQUIRES_SOURCE_OPERATOR"
    assert receipt["strongest_current_reduction"] == "NONE_BEYOND_U(3)_WITHOUT_PAIRED_SOURCE_OPERATOR"
...
def test_connected_second_operator_collapses_relative_phase_ambiguity_conditionally():
...
def test_degenerate_mass_control_does_not_claim_nondegenerate_reduction():
    ...
    mass = receipt["candidates"]["nondegenerate_mass_spectrum"]
    assert mass["target_spectrum_status"] == "DEGENERATE_OR_UNRESOLVED"
